...
//  console.log('segmentTemplate',this.segmentTemplate);
  ...
  try {
    ...
    if (segmentNumber == null) {
        ...
        var version = DataUtils.toString(components[components.length-1])
        var versionlessName = '';
        for (var i=0; i<components.length-1; i++) {
          versionlessName = versionlessName + DataUtils.toString(components[i]) + '/' ;
        }
         
        this.callback(this.ndn, versionlessName, contentObject.content,version);
